<x-pdf.ul start="13">
    <table style="width: 610px">
        <x-pdf.tr setpad="5px" num="1" text="Ikut serta sebagai pelaksana kampanye;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="2"
            text="Menjadi peserta kampanye degan menggunakan atribut partai atau atribut
                Aparatur Sipil Negara;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="3"
            text="Sebagai peserta kampanye dengan mengerahkan Aparatur Sipil Negara lain; dan/atau"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="4"
            text="Sebagai peserta kampanye dengan menggunakan fasilitas Negara."></x-pdf.tr>
    </table>
    <li>Memberikan dukungan kepada calon Presiden / Wakil Presiden dengan cara :</li>
    <table style="width: 610px">
        <x-pdf.tr setpad="5px" num="1"
            text="Membuat keputusan dan / atau tindakan yang menguntungkan atau merugikan salah satu pasangan calon selama masa kampanye; dan / atau"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="2"
            text="Mengadakan kegiatan yang mengarah kepada keberpihakan terhadap pasangan calon yang menjadi peserta pemilu sebelum, selama, dan / atau sesudah masa kampanye meliputi pertemuan, ajakan, himbauan, seruan, atau pemberian barang kepada Aparatur Sipil Negara dalam lingkungan unit kerjanya, anggota keluarga, dan masyarakat."></x-pdf.tr>
    </table>
    <li>Memberikan dukungan kepada calon anggota Dewan Perwakilan Daerah atau Calon Kepala Daerah / Wakil Kepala
        Daerah
        dengan cara memberikan surat dukungan disertai fotocopy Kartu Tanda Penduduk (KTP) atau surat keterangan
        tanda
        penduduk sesuai perundang-undangan; dan</li>
    <li>Memberikan dukungan kepada calon Kepala Daerah / Wakil Kepala Daerah dengan cara :</li>
    <table style="width: 610px">
        <x-pdf.tr setpad="5px" num="1"
            text="Terlibat dalam kegiatan kampanye untuk mendukung calon Kepala Daerah / Wakil Kepala Daerah;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="2"
            text="Menggunakan fasilitas yang terkait dengan jabatan dalam kegiatan kampanye;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="3"
            text="Membuat keputusan dan / atau tindakan yang menguntungkan atau merugikan salah satu pasangan calon selama masa kampanye; dan / atau"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="4"
            text="Mengadakan kegiatan yang mengarah kepada keberpihakan terhadap pasangan calon yang menjadi peserta pemilu sebelum, selama, dan / atau sesudah masa kampanye meliputi pertemuan, ajakan, himbauan, seruan, atau pemberian barang kepada Aparatur Sipil Negara dalam lingkungan kerjanya, anggota keluarga, dan masyarakat."></x-pdf.tr>
    </table>
</x-pdf.ul>

<x-pdf.table num="5"
    text="Selain larangan sebagaimana dimaksud pada ayat (4), <strong>Pihak Kedua</strong> dilarang :">
</x-pdf.table>
<x-pdf.ul>
    <li>
        Melanggar ketentuan jam kerja yang telah disepakati sesuai dengan peraturan kepegawaian yang berlaku;
    </li>
    <li>
        Melanggar ketentuan disiplin kepegawaian yang berlaku dibidang kepegawaian.
    </li>
</x-pdf.ul>

<x-pdf.table num="6"
    text="<strong>Pihak Kedua</strong> yang tidak mematuhi kewajiban dan / atau melanggar larangan sebagaimana dimaksud pada ayat (2), ayat (3), ayat (4) dan ayat (5) diberikan sanksi berupa :">
</x-pdf.table>
<x-pdf.ul>
    <li>
        <b>Hukuman disiplin ringan</b> berupa :
    </li>
    <table style="width: 610px">
        <x-pdf.tr setpad="5px" num="1"
            text="Teguran Lisan apabila tidak melaksanakan kewajiban dan atau tidak masuk kerja tanpa keterangan selama 5 (lima) hari;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="2"
            text="Teguran Tertulis apabila dalam waktu 10 (sepuluh) hari kerja sejak dikeluarkannya Surat Peringatan I masih tetap tidak melaksanakan kewajiban dan atau tidak masuk kerja selama 5 (lima) hari kerja tanpa keterangan;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="3"
            text="Surat Pernyataan tidak puas sekaligus apabila dalam waktu 16 (enam belas) hari kerja sejak dikeluarkannya Teguran Tertulis masih tetap tidak melaksanakan kewajiban dan atau tidak masuk kerja selama 10 (sepuluh hari) kerja tanpa keterangan;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="4"
            text="Ketentuan tidak masuk kerja tersebut dihitung secara komulatif."></x-pdf.tr>
    </table>
    <li>
        <b>Hukuman disiplin sedang</b> berupa :
    </li>
    <table style="width: 610px">
        <x-pdf.tr setpad="5px" num="1"
            text="Pemberhentian hubungan Kerja sementara apabila menjadi tersangka dan ditahan karena melakukan tindak pidana/tersangkut kasus hukum akan dilakukan;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="2"
            text="Apabila dinyatakan bersalah dan telah mempunyai kekuatan hukum tetap, maka <strong>Pihak Pertama</strong> menjatuhkan sanksi Pemutusan Hubungan Kerja terhitung sejak tanggal putusan pengadilan mempunyai kekuatan hukum tetap;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="3"
            text="Apabila dinyatakan tidak bersalah / tidak cukup bukti / bebas baik dalam tingkat penyidikan, penuntutan atau putusan pengadilan yang mempunyai kekuatan hukum tetap, maka apabila masa perjanjian kerja masih berlaku <strong>Pihak Kedua</strong> dipekerjakan kembali;"></x-pdf.tr>
        <x-pdf.tr setpad="5px" num="4" text="Penundaaan Kenaikan Gaji."></x-pdf.tr>
    </table>
    <br> <br>
</x-pdf.ul>
